...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"Finanças Pessoais - v{APP_VERSION} | {connection_mode()}")
        ...
        self.setup_style()
        self.create_layout()
        self.set_cloud_status("Carregando dados...", "warning")
        self.refresh_all()
        self.set_cloud_status("Banco conectado", "success")
    ...
    def setup_style(self):
        ...
        try:
            style.theme_use("flatly" if HAS_TTKBOOTSTRAP else "clam")
        except Exception:
            try:
        ...
        compact_value_font = ("Segoe UI", 13, "bold")
        ...
        style.configure("CompactCard.TFrame", background=self.colors["surface"], relief="flat")

        style.configure("TLabel", background=self.colors["bg"], foreground=self.colors["text"], font=default_font)
        style.configure("Header.TLabel", background=self.colors["surface"], foreground=self.colors["text"], font=title_font)
        style.configure("SubHeader.TLabel", background=self.colors["surface"], foreground=self.colors["muted"], font=("Segoe UI", 9))
        style.configure("White.TLabel", background=self.colors["surface"], foreground=self.colors["text"], font=default_font)
        style.configure("Title.TLabel", font=title_font, background=self.colors["bg"], foreground=self.colors["text"])
        style.configure("CardTitle.TLabel", font=card_title_font, background=self.colors["surface"], foreground=self.colors["muted"])
        style.configure("CardValue.TLabel", font=value_font, background=self.colors["surface"], foreground=self.colors["text"])
        style.configure("CompactTitle.TLabel", font=("Segoe UI", 8, "bold"), background=self.colors["surface"], foreground=self.colors["muted"])
        style.configure("CompactValue.TLabel", font=compact_value_font, background=self.colors["surface"], foreground=self.colors["text"])
        style.configure("CompactValueGreen.TLabel", font=compact_value_font, background=self.colors["surface"], foreground=self.colors["success"])
        style.configure("CompactValueRed.TLabel", font=compact_value_font, background=self.colors["surface"], foreground=self.colors["danger"])

        style.configure("TButton", font=("Segoe UI", 9), padding=(12, 8), borderwidth=0)
        style.configure("Primary.TButton", font=("Segoe UI", 9, "bold"), padding=(14, 8), borderwidth=0)
        style.configure("Secondary.TButton", font=("Segoe UI", 9), padding=(12, 8), borderwidth=0)
        style.configure("Danger.TButton", font=("Segoe UI", 9, "bold"), padding=(12, 8), borderwidth=0)

        # Aparência moderna para tabelas
        style.configure(
            "Treeview",
            font=("Segoe UI", 9),
            rowheight=30,
    ...
    def create_layout(self):
        # Cabeçalho moderno
        top = ttk.Frame(self, style="Header.TFrame", padding=(22, 16))
        top.pack(fill="x", padx=18, pady=(18, 12))

        title_box = ttk.Frame(top, style="Header.TFrame")
        title_box.pack(side="left", fill="x", expand=True)
        ttk.Label(title_box, text="Finanças Pessoais", style="Header.TLabel").pack(anchor="w")
        ttk.Label(
            title_box,
            text=f"v{APP_VERSION} • {connection_mode()}",
            style="SubHeader.TLabel"
        ).pack(anchor="w", pady=(2, 0))
        self.cloud_status_var = tk.StringVar(value="Banco conectado")
        self.cloud_status_label = ttk.Label(title_box, textvariable=self.cloud_status_var, style="CloudSuccess.TLabel")
        self.cloud_status_label.pack(anchor="w", pady=(4, 0))

        actions = ttk.Frame(top, style="Header.TFrame")
        actions.pack(side="right")
        ttk.Button(actions, text="Validar banco de dados", command=self.test_supabase_connection, style="Secondary.TButton").pack(side="left", padx=4)
        ttk.Button(actions, text="Sincronizar base", command=self.push_to_supabase, style="Secondary.TButton").pack(side="left", padx=4)
        ttk.Button(actions, text="Recarregar dados", command=self.sync_supabase, style="Secondary.TButton").pack(side="left", padx=4)
        ttk.Button(actions, text="Restaurar anterior", command=self.restore_previous_state, style="Secondary.TButton").pack(side="left", padx=4)

        self.notebook = ttk.Notebook(self)
        self.notebook.pack(fill="both", expand=True, padx=18, pady=(0, 18))

        self.tab_dashboard = ttk.Frame(self.notebook, padding=14, style="App.TFrame")
        self.tab_entries = ttk.Frame(self.notebook, padding=14, style="App.TFrame")
        self.tab_insights = ttk.Frame(self.notebook, padding=14, style="App.TFrame")
        self.tab_cashflow = ttk.Frame(self.notebook, padding=14, style="App.TFrame")
        self.tab_goals = ttk.Frame(self.notebook, padding=14, style="App.TFrame")
    ...
    def set_cloud_status(self, text, level="success"):
        if not hasattr(self, "cloud_status_var"):
            return
        self.cloud_status_var.set(text)
        style = {
            "success": "CloudSuccess.TLabel",
            "warning": "CloudWarning.TLabel",
            "danger": "CloudDanger.TLabel",
        }.get(level, "CloudSuccess.TLabel")
        self.cloud_status_label.configure(style=style)
        self.update_idletasks()

    def on_close(self):
        try:
            self.set_cloud_status("Finalizando envio pendente...", "warning")
            flush_cloud_writes()
        except Exception:
            pass
        self.destroy()

    def sort_month_values(self, meses):
        def key(m):
            try:
                ano, mes = m.split("-")
                ...
                return (0, 0)
        return sorted([m for m in meses if m and m != "Todos"], key=key, reverse=True)

    def create_dashboard_tab(self):
        cards = ttk.Frame(self.tab_dashboard)
        cards.pack(fill="x", pady=(0, 12))
        self.dash_total = self.compact_card(cards, "Total do mês", "R$ 0,00")
        self.dash_paid = self.compact_card(cards, "Já pago", "R$ 0,00")
        self.dash_open = self.compact_card(cards, "Em aberto", "R$ 0,00")
        self.dash_overdue = self.compact_card(cards, "Atrasado/alerta", "R$ 0,00")
        self.dash_paid.value_label.configure(style="CompactValueGreen.TLabel")
        self.dash_open.value_label.configure(style="CompactValueRed.TLabel")
        self.dash_overdue.value_label.configure(style="CompactValueRed.TLabel")
        for i, card in enumerate([self.dash_total, self.dash_paid, self.dash_open, self.dash_overdue]):
            card.pack(side="left", fill="x", expand=True, padx=(0 if i == 0 else 6, 0 if i == 3 else 6))
        ...
        composition_card = ttk.Frame(left, style="Card.TFrame", padding=14)
        composition_card.pack(fill="both", expand=True, pady=(0, 10))
        ttk.Label(composition_card, text="Composição dos gastos do mês", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        self.dash_composition_frame = ttk.Frame(composition_card, style="Card.TFrame")
        self.dash_composition_frame.pack(fill="both", expand=True)

        paid_card = ttk.Frame(left, style="Card.TFrame", padding=14)
        paid_card.pack(fill="both", expand=True)
        ttk.Label(paid_card, text="Pago x em aberto", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        self.dash_paid_frame = ttk.Frame(paid_card, style="Card.TFrame")
        self.dash_paid_frame.pack(fill="both", expand=True)

        summary_card = ttk.Frame(right, style="Card.TFrame", padding=14)
        summary_card.pack(fill="both", expand=True, pady=(0, 10))
        ttk.Label(summary_card, text="Leitura rápida e ações recomendadas", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        self.dash_text = tk.Text(summary_card, wrap="word", bg="white", relief="flat", font=("Segoe UI", 10), height=12)
        self.dash_text.pack(fill="both", expand=True)

        upcoming_card = ttk.Frame(right, style="Card.TFrame", padding=14)
        upcoming_card.pack(fill="both", expand=True)
        ttk.Label(upcoming_card, text="Gastos em aberto no mês", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        self.dash_upcoming = ttk.Treeview(upcoming_card, columns=("data", "conta", "status", "valor"), show="headings", height=9)
        for col, label, width in [("data", "Data", 90), ("conta", "Conta", 220), ("status", "Status", 160), ("valor", "Valor", 120)]:
            self.dash_upcoming.heading(col, text=label)
            self.dash_upcoming.column(col, width=width, anchor="e" if col == "valor" else "w")
        self.dash_upcoming.tag_configure("alerta", background="#fee2e2", foreground="#991b1b")
        self.dash_upcoming.tag_configure("ok", background="#fff7ed", foreground="#9a3412")
        self.dash_upcoming.pack(fill="both", expand=True)

    def draw_dashboard_charts(self, mes, pago, aberto):
        if hasattr(self, "dash_composition_frame"):
            self.clear_frame(self.dash_composition_frame)
        if hasattr(self, "dash_paid_frame"):
            self.clear_frame(self.dash_paid_frame)

        rows = query("SELECT categoria, valor FROM lancamentos WHERE mes=? AND valor > 0", (mes,))
        agrupado = {}
        for cat, val in rows:
            grupo = "Cartões" if cat in CARTOES else cat
            agrupado[grupo] = agrupado.get(grupo, 0) + float(val or 0)
        top = sorted(agrupado.items(), key=lambda x: x[1], reverse=True)[:6]

        if HAS_MATPLOTLIB:
            fig = Figure(figsize=(4.8, 2.7), dpi=100)
            ...
            if top:
                ...
                ax.text(0.5, 0.5, "Sem gastos lançados", ha="center", va="center")
                ax.axis("off")
            fig.tight_layout()
            c = FigureCanvasTkAgg(fig, master=self.dash_composition_frame)
            c.draw()
            c.get_tk_widget().pack(fill="both", expand=True)

            fig2 = Figure(figsize=(5.4, 3.0), dpi=100)
            ax2 = fig2.add_subplot(111)
            ax2.barh(["Pago", "Em aberto"], [pago, aberto])
            ax2.set_xlabel("R$")
            ax2.grid(True, axis="x", alpha=.25)
            for i, v in enumerate([pago, aberto]):
                ax2.text(v, i, " " + brl(v), va="center", fontsize=8)
            fig2.subplots_adjust(left=0.22, right=0.92, top=0.92, bottom=0.18)
            c2 = FigureCanvasTkAgg(fig2, master=self.dash_paid_frame)
            c2.draw()
            c2.get_tk_widget().pack(fill="both", expand=True)
        else:
            self.draw_canvas_message(self.dash_composition_frame, "Instale matplotlib para ver gráficos.")
            self.draw_canvas_message(self.dash_paid_frame, f"Pago: {brl(pago)} | Em aberto: {brl(aberto)}")

    def refresh_dashboard(self):
        if not hasattr(self, "dash_total"):
            return
        mes = datetime.now().strftime("%Y-%m")
        rows = query("SELECT valor, status_lancamento, categoria FROM lancamentos WHERE mes=?", (mes,))
        total = sum(float(v or 0) for v, _, _ in rows)
        pago = sum(float(v or 0) for v, s, _ in rows if (s or "Pago") in ("Pago", "Débito automático"))
        aberto = sum(float(v or 0) for v, s, _ in rows if (s or "Pago") == "Não pago")
        self.dash_total.value_label.configure(text=brl(total))
        self.dash_paid.value_label.configure(text=brl(pago))
        self.dash_open.value_label.configure(text=brl(aberto))

        atraso = 0
        hoje = date.today()
        abertos = []
        recorrentes = {c["categoria"]: c for c in get_recurring_categories()}
        rows_abertos = query("""
            SELECT categoria, valor, status_lancamento
            FROM lancamentos
            WHERE mes=?
        """, (mes,))
        for cat, valor, stat in rows_abertos:
            stat = stat or "Pago"
            if stat in ("Pago", "Débito automático"):
                continue
            conta = recorrentes.get(cat, {})
            dia = conta.get("dia", 31)
            tipo = conta.get("tipo", "manual")
            venc = due_date_for_month(mes, dia)
            status = status_pagamento(mes, valor, stat, dia, tipo) if venc else stat
            valor_num = float(valor or 0)
            abertos.append((venc, cat, status, brl(valor_num)))
            if venc and venc < hoje:
                atraso += valor_num

        self.dash_overdue.value_label.configure(text=brl(atraso))
        self.draw_dashboard_charts(mes, pago, aberto)

        top_rows = query("""
            SELECT categoria, SUM(valor)
            FROM lancamentos
            WHERE mes=? AND valor > 0
            GROUP BY categoria
            ORDER BY SUM(valor) DESC
            LIMIT 3
        """, (mes,))
        maior = top_rows[0] if top_rows else None
        cartoes = sum(float(v or 0) for c, v in query("SELECT categoria, valor FROM lancamentos WHERE mes=? AND valor > 0", (mes,)) if c in CARTOES)
        pct_cartoes = (cartoes / total * 100) if total else 0

        self.dash_text.configure(state="normal")
        self.dash_text.delete("1.0", "end")
        resumo = [
            f"Mês atual: {mes}",
            f"Total lançado: {brl(total)} | Já pago: {brl(pago)} | Em aberto: {brl(aberto)}",
            "",
            "Ações recomendadas:",
        ]
        if atraso > 0:
            resumo.append(f"• Regularizar primeiro os vencidos: {brl(atraso)}.")
        elif aberto > 0:
            resumo.append("• Existem gastos em aberto no mês. Acompanhe até o vencimento.")
        else:
            resumo.append("• Mês aparentemente regularizado até agora.")

        if maior:
            resumo.append(f"• Maior categoria do mês: {maior[0]} ({brl(maior[1])}).")
        if pct_cartoes >= 40:
            resumo.append(f"• Cartões estão altos: {pct_cartoes:.1f}% do mês. Vale acompanhar semanalmente.")
        elif pct_cartoes >= 25:
            resumo.append(f"• Cartões representam {pct_cartoes:.1f}% do mês. Acompanhe o fechamento.")
        else:
            resumo.append(f"• Cartões representam {pct_cartoes:.1f}% do mês.")

        total_meses = self.monthly_totals()
        if len(total_meses) >= 2:
            ultimos = total_meses[-3:]
            media = sum(v for _, v in ultimos) / len(ultimos)
            if total > media * 1.15:
                resumo.append("• O mês atual está acima da média recente. Revise gastos variáveis.")
            elif total < media * 0.9:
                resumo.append("• O mês atual está abaixo da média recente.")
        self.dash_text.insert("end", "\n".join(resumo))
        self.dash_text.configure(state="disabled")

        for i in self.dash_upcoming.get_children():
            self.dash_upcoming.delete(i)
        for venc, cat, status, valor in sorted(abertos, key=lambda x: x[0] or date.max):
            venc_txt = venc.strftime("%d/%m/%Y") if venc else "-"
            tag = "alerta" if venc and (venc < hoje or "⚠" in status) else "ok"
            self.dash_upcoming.insert("", "end", values=(venc_txt, cat, status, valor), tags=(tag,))
    ...
    def compact_card(self, parent, title, value):
        frame = ttk.Frame(parent, style="CompactCard.TFrame", padding=(14, 10))
    ...
    def months_available(self):
        meses = [r[0] for r in query("SELECT DISTINCT mes FROM lancamentos ORDER BY mes")]
        atual = datetime.now().strftime("%Y-%m")
        meses = self.sort_month_values(meses)
    ...
    def choose_status_dialog(self, current_status):
        """Abre uma janela simples para escolher um status fixo."""
        current_status = (current_status or "").replace("⚠ Vencido sem lançamento", "Não pago").replace("⚠ Não pago vencido", "Não pago").replace("Não lançado", "Não pago")
        result = {"value": None}

        win = tk.Toplevel(self)
        win.title("Editar status")
        ...
        ttk.Label(frame, text="Escolha o novo status", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 12))

        def choose(value):
            result["value"] = value
            win.destroy()

        for text in ["Pago", "Não pago", "Débito automático"]:
            label = f"✓ {text}" if text == current_status else text
            style = "Primary.TButton" if text == current_status else "Secondary.TButton"
            ttk.Button(frame, text=label, command=lambda v=text: choose(v), style=style).pack(fill="x", pady=4)

        ttk.Button(frame, text="Cancelar", command=win.destroy).pack(fill="x", pady=(10, 0))
    ...
    def edit_clicked_payment_cell(self):
        ...
        if col == "valor":
            ...
            novo = self.choose_status_dialog(status)
            if novo is None:
                return
            backup_database("antes_editar_status")
            execute("UPDATE lancamentos SET status_lancamento=? WHERE mes=? AND categoria=?", (novo, mes, old_categoria))
    ...
    def refresh_all(self):
        ...
        if hasattr(self, "insight_start_combo"):
            self.insight_start_combo["values"] = meses
        if hasattr(self, "insight_end_combo"):
            self.insight_end_combo["values"] = meses
    ...
    def refresh_upcoming(self):
        ...
        hoje = date.today()
        mes_ref = self.selected_filter_month() or datetime.now().strftime("%Y-%m")
        total_em_aberto = 0
        itens = []

        # Fonte principal: tudo que está lançado como Não pago no mês.
        rows = query("""
            SELECT categoria, valor, status_lancamento
            FROM lancamentos
            WHERE mes=?
        ...
        for categoria, valor, status in rows:
            status = status or "Pago"
            if status in ("Pago", "Débito automático"):
                continue

            conta = recorrentes.get(categoria, {})
            dia = conta.get("dia", 31)
            tipo = conta.get("tipo", "manual")
            venc = due_date_for_month(mes_ref, dia)
            status_visivel = status_pagamento(mes_ref, valor, status, dia, tipo) if venc else status
            valor_num = float(valor or 0)
            total_em_aberto += valor_num
            itens.append((venc, categoria, status_visivel, brl(valor_num)))

        for venc, cat, status, valor in sorted(itens, key=lambda x: x[0] or date.max):
            venc_txt = venc.strftime("%d/%m/%Y") if venc else "-"
            tag = "alerta" if venc and (venc < hoje or "⚠" in status) else "ok"
            self.upcoming_tree.insert("", "end", values=(venc_txt, cat, status, valor), tags=(tag,))

        if hasattr(self, "upcoming_total_label"):
            self.upcoming_total_label.configure(text=f"Em aberto no mês: {brl(total_em_aberto)}")
    ...
    def test_supabase_connection(self):
        try:
            self.set_cloud_status("Validando banco...", "warning")
            count = test_connection()
            self.set_cloud_status("Banco conectado", "success")
            messagebox.showinfo("Banco de dados", f"Conexão ativa: {connection_mode()}\n\nLançamentos encontrados: {count}")
        except Exception as e:
            self.set_cloud_status("Falha no banco", "danger")
            messagebox.showerror("Banco de dados", f"Falha ao validar conexão.\n\nDetalhe: {e}")

    def push_to_supabase(self):
        if not messagebox.askyesno("Sincronizar base", "Forçar envio dos dados locais para o Supabase?"):
            return
        try:
            self.set_cloud_status("Sincronizando base...", "warning")
            flush_cloud_writes()
            total = sync_local_to_supabase()
            self.refresh_all()
            self.set_cloud_status("Base sincronizada", "success")
            messagebox.showinfo("Supabase", f"Sincronização concluída.\n\nRegistros enviados: {total}")
        except Exception as e:
            self.set_cloud_status("Erro ao sincronizar", "danger")
            messagebox.showerror("Supabase", f"Não foi possível sincronizar.\n\nDetalhe: {e}")

    def sync_supabase(self):
        if not using_supabase():
            messagebox.showwarning("Supabase", "Supabase não configurado. Preencha SUPABASE_URL e SUPABASE_KEY no arquivo .env.")
            return
        try:
            self.set_cloud_status("Recarregando dados...", "warning")
            flush_cloud_writes()
            enviados, recebidos = sync_two_way()
            self.refresh_all()
            if hasattr(self, "refresh_cashflow"):
                self.refresh_cashflow()
            if hasattr(self, "refresh_goals"):
                self.refresh_goals()
            self.set_cloud_status("Dados atualizados", "success")
            erros = last_cloud_errors()
            extra = f"\n\nÚltimos avisos: {erros[-1]}" if erros else ""
            messagebox.showinfo("Supabase", f"Atualização concluída.\n\nEnviados: {enviados}\nRecebidos: {recebidos}{extra}")
        except Exception as e:
            self.set_cloud_status("Erro ao atualizar", "danger")
            messagebox.showerror("Supabase", f"Não foi possível atualizar.\n\nDetalhe: {e}")
    ...
    def generate_text_insights_multi(self, meses, top, detailed=False):
        ...
        if m["total_receitas"]:
            if m["taxa_poupanca"] >= 20:
                lines.append("• Boa margem de sobra. Você está em uma faixa saudável para reserva e investimentos.")
            elif m["taxa_poupanca"] >= 10:
                lines.append("• Margem moderada. Ainda existe sobra, mas vale controlar variáveis.")
            else:
                lines.append("• Baixa margem de sobra. Priorize reduzir gastos variáveis e evitar novas parcelas.")
        else:
            lines.append("• Cadastre receitas para medir saldo e taxa de poupança com mais precisão.")

        lines.append(f"• Fixos representam {m['fixos_pct']:.1f}% das despesas.")
        if m["fixos_pct"] > 60:
            lines.append("  Atenção: fixos muito altos reduzem sua flexibilidade mensal.")
        elif m["fixos_pct"] > 50:
            lines.append("  Fixos relevantes: avalie renegociação de contratos e mensalidades.")

        lines.append(f"• Cartões representam {m['cartoes_pct']:.1f}% das despesas.")
        if m["cartoes_pct"] > 40:
            lines.append("  Atenção: cartão acima de 40% pode dificultar previsibilidade.")
        elif m["cartoes_pct"] > 25:
            lines.append("  Cartões merecem acompanhamento semanal antes do fechamento.")

        if top:
            lines.append("")
            lines.append("Foco de revisão:")
            for cat, val in top[:3]:
                pct = (val / m["total_despesas"] * 100) if m["total_despesas"] else 0
                lines.append(f"• {cat}: {brl(val)} ({pct:.1f}%).")

        lines.append("")
        lines.append("Próximo passo sugerido:")
        lines.append("• Escolha uma categoria grande para acompanhar por 30 dias e defina um limite mensal.")
