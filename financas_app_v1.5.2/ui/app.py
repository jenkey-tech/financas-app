...
from config import APP_VERSION, CATEGORIAS_PADRAO, CATEGORIAS_ACUMULATIVAS, CATEGORIAS_FIXAS, CARTOES, FIXOS, CONTAS_RECORRENTES
from db import (
    init_db,
    query,
    execute,
...
class App(tk.Tk):
    ...
    def create_entries_tab(self):
        ...
        self.card_media = self.compact_card(cards, "Média mensal", "R$ 0,00")
        self.card_maior = self.compact_card(cards, "Maior gasto", "-")
        ...
        ttk.Label(upcoming_box, text="Vencidos e próximos 7 dias", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        ...
        # Linha 4: lançamentos compactos
        table_box = ttk.Frame(self.tab_entries, style="Card.TFrame", padding=10)
        table_box.pack(fill="x")
        ttk.Label(table_box, text="Lançamentos", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 6))
        self.tree = ttk.Treeview(table_box, columns=("mes","categoria","valor","status","obs"), show="headings", height=5)
        for col, label, width in [("mes","Mês",80),("categoria","Categoria",220),("valor","Valor",110),("status","Status",120),("obs","Obs.",300)]:
    ...
    def commit_current_state(self):
        """Cria um snapshot CSV do estado atual e faz commit local no Git.

        Não faz push automaticamente. O envio para GitHub continua sendo controlado
        por você via GitHub Desktop ou pelo comando git push.
        """
        try:
            ...
            messagebox.showinfo(
                "Git",
                f"Commit local criado com sucesso.\n\nSnapshot:\n{exportado}\n\nPara enviar ao GitHub, use o GitHub Desktop ou rode: git push"
            )
